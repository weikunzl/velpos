 ...
 */
export function getSlashCommandContext(text, caretIndex) {
...
export function filterSlashCommandSuggestions(commands, query = '') {
  const q = String(query || '').toLowerCase()
  const invokable = (commands || []).filter((cmd) => (
    cmd
  ...
  return invokable.filter((cmd) => (
    cmd.name.toLowerCase().includes(q)
    || String(cmd.description || '').toLowerCase().includes(q)
  ))
}

export function buildSlashCommandInput(text, context, commandName) {
